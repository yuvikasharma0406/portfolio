...
export default function ProjectCard({
  ...
  return (
    <motion.article
      ...
      className={cn(
        "bg-white rounded-3xl border border-[#E7E3DA] p-6 sm:p-10 lg:p-14 transition-all duration-300 group",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14 items-start">
        ...
        <div className="lg:col-span-4 space-y-6 pb-6 lg:pb-0 border-b lg:border-b-0 lg:border-r border-[#F0ECE4] lg:pr-10">
          ...
          <div className="pt-2">
            ...
            >
              <Link
                href={`/work/${project.slug}`}
                className="inline-flex items-center gap-1.5 min-h-[44px] text-xs font-semibold uppercase tracking-wider text-[#121316] group-hover:text-[#FF3E1D] transition-colors duration-200"
              >
        ...
        <div className="lg:col-span-8 space-y-5 sm:space-y-6">
          <Link href={`/work/${project.slug}`} className="block">
            <h3 className="text-2xl sm:text-3xl lg:text-4xl font-display font-normal text-[#121316] group-hover:text-[#FF3E1D] transition-colors duration-300 leading-[1.2] break-words">
              {project.title}
            </h3>
          </Link>

          <p className="text-sm sm:text-base lg:text-lg text-[#5F6368] leading-relaxed font-normal">
            {project.summary}
          </p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            <div className="p-4 sm:p-5 rounded-2xl bg-[#FAF8F5] border border-[#E7E3DA] transition-colors duration-200 group-hover:border-[#E0DDD5]">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {project.tags.map((tag, tIdx) => (
              <span
                key={tIdx}
                className="text-[11px] sm:text-xs px-3 py-1 rounded-full bg-[#FAF8F5] border border-[#E7E3DA] text-[#5F6368] transition-colors duration-200 hover:border-[#121316] hover:text-[#121316]"
              >
                #{tag}
              </span>
            ))}
